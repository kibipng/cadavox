...
func _on_speech_to_text_transcribed_msg(is_partial: Variant, new_text: Variant) -> void:
	...
	if main_player and main_player.has_method("process_speech_for_word_gun"):
		main_player.process_speech_for_word_gun(new_text)

# Add these functions to your main.gd

# Letter projectile system
func spawn_letter_projectile(letter: String, position: Vector3, direction: Vector3, shooter_steam_id: int):
	# Create letter projectile (similar to regular letters but with physics)
	var letter_projectile = preload("res://scenes/letter_projectile.tscn").instantiate()
	
	letter_projectile.global_position = position
	letter_projectile.letter_text = letter
	letter_projectile.shooter_id = shooter_steam_id
	
	# Set velocity
	letter_projectile.linear_velocity = direction * 20.0  # Fast projectile
	
	# Add to scene
	add_child(letter_projectile)
	
	print("Spawned letter projectile: ", letter, " from player ", shooter_steam_id)

# Blue shell system
func spawn_blue_shell_projectile(shell_data: Dictionary):
	var blue_shell = preload("res://scenes/blue_shell.tscn").instantiate()
	
	var start_pos = shell_data["start_position"]
	blue_shell.global_position = Vector3(start_pos[0], start_pos[1], start_pos[2])
	blue_shell.target_steam_id = shell_data["target_steam_id"]
	blue_shell.launcher_steam_id = shell_data["launcher_steam_id"]
	
	add_child(blue_shell)
	
	print("Blue shell launched targeting player ", shell_data["target_steam_id"])

# Message handlers for projectiles
func handle_letter_projectile_spawn(projectile_data: Dictionary):
	var pos = projectile_data["position"]
	var dir = projectile_data["direction"]
	spawn_letter_projectile(
		projectile_data["letter"],
		Vector3(pos[0], pos[1], pos[2]),
		Vector3(dir[0], dir[1], dir[2]),
		projectile_data["shooter_steam_id"]
	)

func handle_blue_shell_launch(shell_data: Dictionary):
	spawn_blue_shell_projectile(shell_data)
